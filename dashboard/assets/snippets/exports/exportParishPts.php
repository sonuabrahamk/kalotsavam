...
$sql = "SELECT md.PARISH, md.FORANE, SUM(md.RANK_POINTS) AS F_POINTS, SUM(md.GRADE_POINTS) FROM `master_data` md JOIN `events` e on e.SECTION = md.SECTION WHERE e.STATUS='PUBLISHED' GROUP BY md.PARISH, md.FORANE ORDER BY SUM(md.RANK_POINTS) DESC, SUM(md.GRADE_POINTS) DESC, PARISH ASC;";
